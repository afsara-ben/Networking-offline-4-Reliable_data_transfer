...
int base_A; //to maintain sender's lowest un-acknowledged packet, start timer for this packet.
...
/*  printf("\n ...in A_input() ... \n\n");
  ...
  //do nothing if checksum doesn't match
  ...
  if (packet.acknum < base_A) {
        //printf("**** At %f in A_input() : got NACK ack%d  ****\n", time, packet.acknum);
        return;
    }
  ...
  { 
    ...
    if(base_A == nextseqnum_A)          //stop timer when ackX sent for pktX
        {
          stoptimer(0);
